...
import Messages from '../messagechatmodule/Messagechatmodule';
import CreateGroupChat from '../../components/Chat/components/CreateGroupChat';
import SelectUsers from '../../components/Chat/components/SelectUsers';
import GroupName from '../../components/Chat/components/GroupName';
import { _io } from '../../services/socket.service';
import ComposeDropdown from '../../components/Chat/Composedropdown';
...
import SupportMessages from '../messagechatmodule/Supportchatmodule';
...
const Moblileviewchatmodule = ({ togggle, reciever, setReciever }: IProps) => {
    ...
    useEffect(() => {
        console.log('Gropus fsdkjdsa');
        setGroup(1);
    }, []);

    useEffect(() => {
        if (chat.newRoom) {
            setOpenChat(chat.newRoom);
            setGroup(5);
    ...
    return (
        <div
            className={`
             ...
             bg-[#1E1E2C] z-40 rounded-r-none lg:rounded-tl-2xl hover:bg-[#1c1c27]   border-b-0 w-full lg:hidden  block h-[100vh]  overflow-auto !focus-visible:outline-none focus-visible:border-0 `}>
            <div
                className={`${
                    state ? ' overflow-y-scroll  opacity-100' : 'h-0  opacity-0'
                } relative duration-300 AtScroll scrollbarHide at-sidebarwrapper overflow-hidden !focus-visible:outline-none !focus-visible:border-0 `}>
                {(() => {
                    switch (group) {
                        case 1: {
                            return (
                                <div className="relative">
                                                ...
                                                <div className="w-full">
                                                    <div className="flex justify-between gap-2">
                                                        <h6 className="flex flex-shrink-0 flex-col text-[#ff5500] text-[18px] font-Proxima-Bold">
                                                            Support
                                                            <span className="block text-[14px] text-white font-Proxima-Regular w-[20rem] xs:w-[12rem] truncate ">
                                                        ...
                                                        chat?.roomUsers
                                                            ...
                                                            .map((ele: any, i: number) => {
                                                                return ele._id === user?.userId ? null : (
                                                                    <div key={i}>
                                                                        {/* <Link legacyBehavior href="/messagechatview">
                                                                            <a> */}
                                                                        <div
                                                                            onClick={() => {
                                                                                setOpenChat(chat);
                                    ...
                                    {/* <button
                                        className="bg-themecolor text-[#20242a]     fixed bottom-44 right-16 rounded-full px-5 py-3 flex items-center justify-center text-[14px] font-Proxima-SemiBold shadow-[0_0_30px_0_rgba(241,207,74,0.5)"
                                        type="button"
                                        onClick={() => {
                                            setGroup(2);
                                        }}>
                                        ...
                                        <span className="pl-[8px] ">New Group</span>
                                    </button> */}
                                    <ComposeDropdown responsive={true} setGroup={setGroup} />
                                </div>
                            );
                        }
                        case 2: {
...
export default Moblileviewchatmodule;
