...
import MentionedMessage from './Mention';
import Messagedrodpdown from '../Dropdown/Messagedropdown';
...
const Messages = ({ data1, closeHandler, index, openChatHeadHandle, openedChatHead, setClose, close }: IProps) => {
    ...
    const [viewMember, setViewMemember] = useState<boolean>(false);
    ...
    let focusInput: any = useRef(null);
    const focusTheInput = () => {
        if (document?.getElementById('TYPE__')) document?.getElementById('TYPE__')?.focus();
    ...
    useEffect(() => {
        if (data && openedChatHead.some((item: any) => item._id === data1._id)) {
            dispatch(seenMessage({ roomId: data?._id }));
        }
    }, [newMessage, openedChatHead.some((item: any) => item._id === data1._id)]);
    ...
    useLayoutEffect(() => {
        if (focusInput.current) {
            ...
            focusInput.current?.addEventListener('keyup', (event: any) => {
                // console.log({ event });
            });
        }
    ...
    function urlify(text: string) {
        ...
        return text?.replace(urlRegex, function (url) {
            return userMentions?.some((value: any) => value?.userName == url.split('@')[1])
                ? `<a  target="_blank" href=profile/${url.split('@')[1]} id=${url}>${url} </a>`
                : url;
        });
    }
    const messageHanlder = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (message.trim() == '' && !file) {
            ...
            // var urlRegex = /(@?:\/\/[^\s^\<]+)/g;
            // var urlRegex = /(\@(.*?)\.)/g;
            // console.log('test', urlRegex);
            // toast.error(urlRegex);
            // newText?.replace(urlRegex, function (url: string) {
            //     return `<a  target="_blank" href=/profile/${url} id=${url}>  @${url} </a>`;
            // });
    ...
    return (
        <div
            // onLoad={updateScroll}
            key={data1?._id}
            className={`bg-[#14141F] cahtparent border-blue1 hover:bg-[#1c1c27]  rounded-lg w-[22.938rem] relative `}>
            <div className="cursor-pointer bg-[#2B2B35]  rounded-t-lg text-center h-16  flex justify-between px-4 items-center">
                <div
                    onClick={() => {
                        openChatHeadHandle(data1);
                    }}
                                            ...
                                            <>
                                                <figure className="w-[32px] h-[32px]  relative overflow-hidden rounded-full UerProfileImage AtchatUserprofile flex items-center justify-center">
                                                    {ele?.avatar ? (
                                                        ...
                                                        <p className="w-full h-full !bg-themecolor pt-1 hover:!bg-[#cdab3a]   flex items-center justify-center rounded-full text-black1 text-2xl">
                                                ...
                                                <div className="ml-2 ">
                                                    <p className="text-white text-[16px] text-left   Atpricehoverholder   leading-tight font-Proxima-Bold flex justify-center">
                                                        <span className="max-w-[7.10rem] hover:text-themecolor  truncate   ">
                                                            {' '}
                                                            {ele.firstName + ' ' + ele.lastName}
                                                        </span>
                                                        <span
                                                            className="-mt-4 text-white hover:text-themecolor Atpricehover2"
                    ...
                    {data.type == 'GROUP' && (
                        <>
                            <a className="flex items-center">
                                <figure className="w-[32px] h-[32px] relative overflow-hidden  rounded-full UerProfileImage flex items-center justify-center">
                                    {data?.image ? (
                                        ...
                                        <p className="flex items-center justify-center w-full h-full pt-1 text-2xl hover:!bg-white  rounded-full bg-themecolor text-black1">
                            ...
                            <div className="ml-2">
                                <p className="flex text-base text-white hover:text-themecolor braek justify-left font-Proxima-Bold">
                ...
                <div className="flex items-center mr-2 ">
                    {data.type == 'PRIVATE' && (
                        <div
                            className={`${blockedUser}?"":cursor-pointer AtCallIconHover`}
                            onClick={async () => {
                                if (!blockedUser) {
                                        ...
                                        .then(function (stream: any) {
                                            ...
                                            if (stream.getAudioTracks().length > 0) {
                                                ...
                                                setDataa({
                                                    ...
                                                    reciver: data?.roomUsers.find((el: any) => el._id !== user?.userId)
                                                        ?._id
                                                });
                                                setPopup(true);
                                                setstatepop(73);
                                    ...
                                    //             console.log('permissions0 camera');
                ...
                <div className="relative top-0 left-0 flex items-center ">
                    <div className="relative ">
                        {openedChatHead.some((item: any) => item._id === data1._id) && (
                            <Deletedropdown
                                ...
                                setViewMemember={setViewMemember}
                    ...
                    <i
                        ...
                        className={`icon-close bg-[#363642] h-7 w-7 flex justify-center mr-2 text-2xl items-center  rounded-full AtChatCrossIcon  text-[#7C7EBB]`}>
            ...
            <div
                className={`${
                    openedChatHead.some((item: any) => item._id === data1._id)
                        ? 'h-[448px] overflow-y-auto flex'
                        : 'h-0  hidden'
                }   flex flex-col justify-end `}>
                <div
                    className={`${
                        openedChatHead.some((item: any) => item._id === data1._id)
                            ? 'h-[50vh] overflow-y-auto opacity-100'
                            : 'h-0  opacity-0'
                    } bg-[#14152 relative at-sidebarwrapper scrollbarHide scrolHide duration-300 AtScroll overflow-hidden flex flex-col-reverse`}
                    {...getRootProps({
                        onClick: (event) => event.stopPropagation()
                    })}>
                    <div
                        id={'yourDivID' + '-' + data?._id}
                        className="h-[50vh] overflow-y-auto  scrolHide  scrollbarHide w-full flex at-sidebarwrapper flex-col-reverse justify-start">
                        <div className="">
                                                                      ...
                                                                      <div className="flex items-center  justify-end gap-1.5 relative">
                                                                          {el?.type == 'text' && (
                                                                              <>
                                                                                  ...
                                                                                  />
                                                                                  <span className="AtChatReplyIcon">
                                                                                      <svg
                                                                                          width="20"
                                                                                          height="15"
                                                                                          viewBox="0 0 20 15"
                                                                       ...
                                                                       max-w-[270px] round-yellow Atmymessage   font-Proxima-Regular `}>
                                                                              {el?.reply && (
                                                                                  <p
                                                                              ...
                                                                              <p
                                                                                  ...
                                                                                  {/* {el?.body && <SingleMessage data={el?.body} />} */}
                                                                              </p>
                                                                          </span>
                                                                      </div>
                            ...
                            <div
                                className={`${
                                    openedChatHead.some((item: any) => item._id === data1._id) ? '' : 'hidden'
                                }  flex justify-between bottom-0 items-end relative w-[22.938rem] z-[2]   `}>
                                {mentionUi && (
                                    <div className="absolute z-10 w-full p-2 space-y-1 bottom-20 ">
                        ...
                        {typing && typingUserName != user?.userName && (
                            <div
                                className={`w-full bg-[#14141f]  absolute z-[2] ${
                                    file ? 'bottom-[115px]' : 'bottom-[75px]'
                                }  left-0 border-t-[1px] border-[#262639] `}>
                                <p className={` text-left text-[12px] pl-4 py-2 `}>{typingUserName} is typing ....</p>
                            </div>
                        )}
                    </div>
                </div>
                    ...
                    {/* {data1?.type == 'PRIVATE' && (
                                    <>
                                        <Input
                                            ...
                                            styles=" !px-0   !py-0   !border-[#2B2B35]      !rounded-[50px] "
                                            className="px-6 pr-[45px] pl-[12px]    !py-3 placeholder:text-sm placeholder:text-[#89898F]  !rounded-[50px]"
                    ...
                    <div className="relative">
                        {reply && (
                            <div className=" pl-4 pr-6 py-4   bg-[#14141F] hover:bg-[#25252c] w-full border-transparent  border border-t-[#2B2B35] text-base font-Circular-Book  outline-none  text-white leading-[0] ">
                                <p
                                    className="text-[#b8b8bc] text-sm mt-5 mx-1 truncate  w-[19rem] !break font-Proxima-Regular "
                                    dangerouslySetInnerHTML={{
                        ...
                        <div className="pl-[40px] pr-[34px] relative ">
                            ...
                            <div className="relative">
                                <TextArea
                                    ...
                                    className="px-6 pr-[48px] pl-[12px] !py-3  placeholder:text-[#89898F]  !rounded-[10px]"
                                    placeholder={isDragActive ? 'Drop the files here ...' : 'Type your message'}
                                />
                                {openedChatHead.some((item: any) => item._id === data1._id) && (
                                    <div className="">{emojo}</div>
                                )}
                            </div>
                            <p
            ...
            <div className="absolute top-0 bottom-0 right-0 z-30 flex flex-row-reverse items-end gap-5 shadow-2xl">
                {/* {viewMember && (
          <Members data1={data} setViewMemember={setViewMemember} />
        )} */}
            </div>
        </div>
    );
