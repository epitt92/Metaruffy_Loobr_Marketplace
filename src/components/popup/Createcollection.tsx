...
import { createCollection } from '../../redux/collections/actions';
import { generateFileUrl, uploadFile } from '../../services/upload.service';
import Button from '../Button/Button';
import Input from '../input/Input';
import Poups from './poups';

export const Createcollection = ({ setstate }: any) => {
    ...
    const [feedFile, setfeedFile] = useState<any>(null);
    const [fileName, setFileName] = useState<any>('');
    const [type, setType] = useState<any>('');
    const loading = useSelector((state: any) => state.collections.createCollectionsLoading);
    ...
    // const selectLogo = useCallback((e: any) => {
    //   setLogoImage(e.target.files[0]);
    //   setLogoPreview(URL.createObjectURL(e.target.files[0]));
    // }, []);
    // useEffect(() => {
    //     if (feedFile) {
    //         selectCover(feedFile);
    //         setfeedFile(null);
    //     }
    // }, [feedFile]);
    ...
    const handleSubmit = (e: React.ChangeEvent<HTMLFormElement>) => {
        ...
        if (values.videoUrl && !urlRegex.test(values.videoUrl)) {
            ...
            dispatch(createCollection(data, setstate));
        }
    };
    ...
    return (
        <div className=" sm:w-auto w-[450px] xs:w-full m-auto rounded-lg px-8 py-6 ">
            <form onSubmit={handleSubmit} key="create-collection">
                <h3 className="xl:text-2rem text-white">Create Collection</h3>
                        ...
                        <label className="relative cursor-pointer block border-2 sm:w-auto   lg:w-[200px] h-[200px] rounded-2xl  border-dashed border-[#2B2B35] ">
                            {loadingUploadLogo ? (
                                ...
                                <>
                                    ...
                                    />
                                    {logoPreview ? (
                                        <figure className="m-auto xl:m-0 w-[200px] h-[200px] Atcreatecollectionimg">
                                            <Image
                                                width={200}
                                                height={200}
                                ...
                                <>
                                    ...
                                    {coverPreview ? (
                                        ...
                                        <div className="flex flex-col justify-center items-center w-[100%] h-[100%]">
                                            ...
                                            <span className="text-[#8D8C8C] text-xs"> (1920 x 350 Recommended)</span>
                                        </div>
                                    )}
                                </>
                            )}
                ...
                />
                <p className="text-white xl:text-lg mt-10 mb-3">Maximum number of NFTs*</p>
                <Input
                    className=" placeholder:!text-[#A1A1A5] "
                    placeholder="12"
                    name="numberOfNfts"
                ...
                <div className="relative">
                    <Input
                        ...
                        disabled={true}
                    />
                    <label className="cursor-pointer block absolute right-0 top-0 h-full">
                        <>
                            <input
